...
def checkuser(email):
    cur = con.cursor()
    cur.execute("SELECT id_account from accountapi where email='"+str(email)+"'")
    rows = cur.fetchall()
    if len(rows) !=0:
        return True
    else:
...
def checkpass(email,password):
    cur = con.cursor()
    cur.execute("SELECT id_account from accountapi where password='"+str(password)+"' and email='"+str(email)+"'")
    rows = cur.fetchall()
    if len(rows) !=0:
        return True
    else:
...
def getusername(email):
    cur = con.cursor()
    cur.execute("SELECT username from accountapi where email='"+str(email)+"'")
    rows = cur.fetchall()
    if len(rows) !=0:
        return rows[0][0]
    else:
...
def getuserrole(email):
    cur = con.cursor()
    cur.execute("SELECT role from accountapi where email='"+str(email)+"'")
    rows = cur.fetchall()
    return rows[0][0]
...
@app.route('/quan/post/edit/<int:post_id>',methods=['POST'])
def nhatquan_post_edit(post_id):
